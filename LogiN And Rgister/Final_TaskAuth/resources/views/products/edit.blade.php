            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('product.update', $product->Product_id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="my-3">
                            <input type="text" name="Product_name" value="{{ old('Product_name', $product->Product_name) }}" class="border-gray-300 shadow-sm w-1/2 rounded-lg" placeholder="Enter Product Name">
                            @error('Product_name')
                        ...
                        <div class="my-3">
                            <input type="number" name="Price" value="{{ old('Price', $product->Price) }}" class="border-gray-300 shadow-sm w-1/2 rounded-lg" placeholder="Enter Price">
                            @error('Price')
                                <p class="text-red-400 font-medium">{{ $message }}</p>
                            @enderror
                        </div>
